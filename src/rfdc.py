...
class RFDC(object):
    ...
    def from_device_info(
        cls, parent, device_name, device_info, initialise=False, **kwargs
    ):
        """
  ...
  """
    ...
    def apply_dto(self, dtbofile):
        ...
        reply, informs = t.katcprequest(
            name="dto", request_timeout=t._timeout, request_args=args
        )
    ...
    def del_clk_file(self, clkfname):
        ...
        args = (clkfname,)
        reply, informs = t.katcprequest(
            name="delbof", request_timeout=t._timeout, request_args=args
        )
        return True
    ...
    def upload_clk_file(self, fpath, port=None, force_upload=False):
        ...
        if not force_upload:
            ...
            if clkfiles.count(fname) == 1:
                print(
                    "file exists on remote filesystem, not uploading. Use `force_upload=True` to overwrite."
                )
                return True
    ...
    def progpll(self, plltype, fpath=None, upload=False, port=None):
        ...
        reply, informs = t.katcprequest(
            name="rfdc-progpll", request_timeout=t._timeout, request_args=args
        )
    ...
    def set_dsa(self, ntile, nblk, atten_dB):
        ...
        reply, informs = t.katcprequest(
            name="rfdc-set-dsa", request_timeout=t._timeout, request_args=args
        )
