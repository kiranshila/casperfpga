...
class CasperFpga(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # This is only used for aiokatcp for now, but moving other
        # things over to aio will use it
    ...
    def choose_transport(self, host_ip, port):
        ...
        try:
            ...
            raise RuntimeError("Address/host %s makes no sense to " "the OS?" % host_ip)
    ...
    def read(self, device_name, size, offset=0, **kwargs):
        ...
        if self.is_little_endian:
            assert (
                len(data) % 4
            ) == 0, "Can only read multiples of 4 bytes because CasperFpga is doing an endianness flip"
    ...
    def blindwrite(self, device_name, data, offset=0, **kwargs):
        if self.is_little_endian:
            assert (
                len(data) % 4
            ) == 0, "Can only write multiples of 4 bytes because CasperFpga is doing an endianness flip"
            # iterate through 32-bit words and flip them
            data_byte_swapped = b""
            for i in range(0, len(data), 4):
                data_byte_swapped += data[i : i + 4][::-1]
            return self.transport.blindwrite(
                device_name, data_byte_swapped, offset, **kwargs
            )
    ...
    def upload_to_ram_and_program(
        self, filename=None, wait_complete=True, initialise_objects=False, **kwargs
    ):
        """
            ...
            if self.bitstream:
                if self.bitstream[-3:] == "fpg":
                    self.get_system_information(
                        filename, initialise_objects=initialise_objects
                    )
    ...
    def read_dram(self, size, offset=0):
        ...
        self.logger.debug(
            "Reading a total of %8i bytes from offset %8i..." % (size, offset)
        )
        while n_reads < size:
            ...
            local_data = self.dram_bulkread("dram_memory", local_reads, local_offset)
            data.append(local_data)
            self.logger.debug(
                "Reading %8i bytes from indirect "
    ...
    def write_dram(self, data, offset=0):
        ...
        self.logger.debug(
            "Writing a total of %8i bytes from offset %8i..." % (size, offset)
        )
        ...
        while n_writes < size:
            ...
            self.logger.debug(
                "Writing %8i bytes from indirect address %4i "
                "at local offset %8i..." % (local_writes, dram_page, local_offset)
            )
            if last_dram_page != dram_page:
                self.write_int("dram_controller", dram_page)
                last_dram_page = dram_page
            self.blindwrite(
                "dram_memory", data[n_writes : n_writes + local_writes], local_offset
            )
    ...
    def write(self, device_name, data, offset=0):
        ...
        if new_data != data:
            ...
            err_str = (
                "Verification of write to %s at offset %d failed. "
    ...
    def write_int(self, device_name, integer, blindwrite=False, word_offset=0):
        ...
        try:
            ...
            self.logger.error(
                "Writing integer {} failed with error: {}".format(integer, ve)
            )
            raise ValueError(
                "Writing integer {} failed with error: {}".format(integer, ve)
            )
        ...
        self.logger.debug(
            "Write_int %8x to register %s at word offset %d "
            "okay%s."
            % (integer, device_name, word_offset, " (blind)" if blindwrite else "")
        )

    def _create_memory_devices(
        self, device_dict, memorymap_dict, legacy_reg_map=True, **kwargs
    ):
        """
        ...
        for device_name, device_info in list(device_dict.items()):
            ...
            if device_name == "":
                raise NameError(
                    "There's a problem somewhere, got a blank " "device name?"
                )
            ...
            try:
                ...
                if not isinstance(known_device_class, collections.Callable):
                    raise TypeError(
                        "%s is not a callable Memory class - "
        ...
        for name, memdevice in memorymap_dict.items():
            ...
            if name in device_dict.keys():
                ...
                if name in self.memory_devices:
                    raise NameError(
                        "Device named %s is already in memory device list" % name
                    )
                self.memory_devices[name] = Memory(
                    name, 32, memdevice["address"], memdevice["bytes"]
                )
    ...
    def device_names_by_container(self, container_name):
        ...
        return [
            devname
            for devname, container in self.memory_devices.items()
    ...
    def get_system_information(
        self, filename=None, fpg_info=None, initialise_objects=False, **kwargs
    ):
        """
        ...
        self._create_memory_devices(
            device_dict, memorymap_dict, initialise=initialise_objects
        )
    ...
    def _add_sys_registers():
        ...
        sys_registers = {
            "sys_board_id": standard_reg.copy(),
                ...
                if "git_info_found" in list(git_info[filename].keys()):
                    if git_info[filename]["git_info_found"] == ["1"]:
                        rv.append(
                            (
                                filename,
                                git_info[filename]["commit_hash"][0]
                                + "_"
                                + git_info[filename]["status"][0],
                            )
                        )
                    continue
